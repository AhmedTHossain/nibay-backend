...
export async function POST(request: Request) {
  try {
    ...
    // Update OTP and deviceID in the database
    const updateData = {
      deviceID,
      otpCode,
      lastOTPRequestTime: new Date(), // Update the time for reference
      otpRequestCount: 1 // Reset count since checks are removed
    };
